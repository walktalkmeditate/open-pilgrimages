 ...
 * at the target path. Route detail pages are hand-authored, and a route id can
 * become a pilgrimage id: docs/kumano-kodo.html moved aside to
 * kumano-kodo-nakahechi.html, and docs/shikoku-88.html was deleted when its
 * route split into the four dōjō sections. Without this the generator would
 * overwrite a 38 KB hand-written page with a 5 KB list, and the loss would
 * only surface on the next CI step, in a message telling the maintainer to
 * commit the result.
 */
