...
/** Reads and filters one checklist file off disk, or reports it absent. */
function readChecklist(root: string, id: string, qualifier: string): ReviewChecklist | undefined {
  ...
  return buildReviewChecklist(`docs/review/${id}.md`, readFileSync(path, "utf8"), qualifier);
}
 ...
 * The file-selection rule itself lives in review-checklist.ts, shared with
 * check-drafted-diff.ts; this wrapper only supplies the filesystem reads that
 * a pure selector cannot do itself.
 */
function reviewChecklist(
  ...
  const pilgrimage = pilgrimageId
    ...
    : undefined;
  return selectReviewChecklist(section, pilgrimage, indices);
}
