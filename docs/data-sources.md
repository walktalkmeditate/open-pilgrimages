...
**The 21-year walking-completion series:** what the bureaus publish is measured over the whole circuit, not per province, and the Shikoku 88 is four sections with no directory of its own to hold a file. So the series lives in the `pilgrimage.stats` block of each dōjō's `routes/shikoku-88-{awa,tosa,iyo,sanuki}/metadata.json` — repeated identically in all four, the way `name`, `kind` and `circular` already are, with `npm run validate` refusing sections that disagree. Its `dataNote` is what says the figures are the circuit's and not the dōjō's, which is why the schema requires it.

`npm run build-index` lifts a summary of that block to `pilgrimages[]` in `index.json` — `lastUpdated`, `dataYear`, `dataNote` and `annualPilgrims.latest`, and nothing more. The registry is the catalog every consumer downloads before anything else, and `pilgrimages[]` is there so one can see what a pilgrimage is without fetching its sections; twenty-one years of per-year counts is not that, and no route's own `stats.json` is copied into the registry either. Fetch the series from any section that carries it.
