...
**Files:**
- Create: `scripts/check-drafted-diff.ts`, `scripts/review-checklist.ts`
- Test: `scripts/check-drafted-diff.test.ts`
- Modify: `.github/workflows/validate.yml`, `package.json` (scripts), `scripts/validate.ts`
...
A post-implementation review pass found this task's first draft ignoring fence/blockquote stripping and file-scoping for the checklist's two tick forms, and swallowing every `git show` failure as "nothing to strip." The fence-stripping and per-file scoping above come from spec section 6, not from this plan's own drafting; the shared module they live in is `scripts/review-checklist.ts` — pulled out of `scripts/validate.ts` so both it and the new gate read the same rule — and the `showAtRef` error handling shown in Steps 3 and 5 are the corrected versions. A later task or PR that extends either gate's checklist handling should change the rule in `scripts/review-checklist.ts`, not copy it into a second file.
