...
`waypoints.geojson` keeps every place OSM offered. `build-ways` decides which of them becomes a moment. Nothing is destroyed, the decision is reversible by editing one rule, and the site's waypoint counts — which `check-site` reads back out of README and the section pages as prose — stay true without a single page being rewritten.
...
- it carries `bangaiNumber` — the 20 bangai of §3.4;
...
|---|---|---|
| today | 298 | 7.5 |
| under the rule (88 + 20 bangai + 42) | 150 | **3.8** |
...
Twenty bangai (番外) temples enter as curated waypoints with `source: "curated"`, which `schema/waypoints.schema.json:207-210` already sanctions, carrying `bangaiNumber` alongside the same fields a fudasho carries. They are the only new waypoints this work adds, so they are the only reason a waypoint count moves — README and the four section pages are regenerated by `build-assets` and re-checked by `check-site` for exactly that reason.
...
- **`check-site` will fail until the pages are regenerated.** It reads waypoint totals from `README.md:5` and per-section figures out of `docs/shikoku-88-{iyo,tosa,sanuki}.html`. Adding twenty bangai moves those numbers.
