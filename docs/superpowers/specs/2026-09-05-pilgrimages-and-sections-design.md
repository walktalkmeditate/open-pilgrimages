...
**Precondition discharged, 2026-09-08.** Both sweeps were run against the OSM API and both sections were cut under §4.3, metadata-only. The results are recorded here, and not only in each section's `metadata.json`, because this spec is what the next attempt reads:

- **Iseji — relation [19693803](https://www.openstreetmap.org/relation/19693803).** Confirmed by `wikidata=Q11379141`, `wikipedia=ja:伊勢路 (熊野古道)` and `website=https://www.kodo.pref.mie.lg.jp/`, Mie Prefecture's own Kumano Kodō site. The id is right and the geometry behind it is not there: the relation holds two ways and nothing else — [120679201](https://www.openstreetmap.org/way/120679201), 1.32 km at 136.20 E / 34.09 N in the hills above Owase, and [1128207928](https://www.openstreetmap.org/way/1128207928), 0.74 km at 136.11 E / 33.90 N above Kumano-shi — 2.06 km against a ~170 km route, their nearest ends 22.3 km apart. A corridor-wide sweep for the classic Iseji pass names adds only about 10 km of loose, unrelated ways. Not a wrong id, an upstream gap: the id is pinned in `routes/kumano-kodo-iseji/metadata.json` so the next attempt starts from a confirmed fact, and the section waits for OpenStreetMap.
- **Ohechi — no relation exists.** A sweep of every `route=hiking|foot|pilgrimage` relation across the whole Tanabe → Kushimoto → Nachi corridor returned seven relations, all seven Nakahechi or Kohechi; nothing is named or tagged for the Ohechi. The trail is in OpenStreetMap only as five loose, unrelated ways inside Tanabe totalling 0.58 km: [298987443](https://www.openstreetmap.org/way/298987443) (`name:en="Kumano Kodo Ohechi Route"`) plus 300025709, 300025708, 129180109 and 129185848. No id is recorded for this section because there is none to record — inventing one, or pinning a neighbouring route's, would misrepresent what OSM holds. Do not repeat this search; check whether a relation has been created upstream instead.
