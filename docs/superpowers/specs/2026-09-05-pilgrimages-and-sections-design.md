...
|---|---|---|
| Pilgrimage | A grouping with an id and a name, in walking order of its sections | `index.json` `pilgrimages[]`, declared by each section's `metadata.json` |
...
A pilgrimage is index-level only; it has no directory. Every route directory is a section. Pilgrimage ids and section ids are separate namespaces and never collide.

### 2.1 Ids and directories

| Pilgrimage id | Name | Sections, in walking order |
|---|---|---|
...
```json
"pilgrimage": { "id": "kumano-kodo", "name": { "en": "Kumano Kodō", "ja": "熊野古道" }, "order": 1 },
"osm": { "relations": [17131166] }
```

`pilgrimage.order` is the section's position in the pilgrimage's walking order (`legs`) or listing order (`alternatives`). `osm.relations` becomes required for any section that has a `ways/` package; `osm.query` alone is no longer enough to build a walked line (§4.3). `parentRouteId` stays for the coastal variant only.
...
`build-index` derives `pilgrimages[]` from the sections' metadata (ordered by `pilgrimage.order`), and `check-site` verifies every section's pilgrimage exists and every pilgrimage has at least one section. `schema/index.schema.json` gains `pilgrimages` and the per-route `pilgrimage` string.
...
`validate` checks, for each `legs` pilgrimage, that section N's last stage `end` is within `SNAP_METERS` of section N+1's first stage `start`, naming both sections on failure. `alternatives` pilgrimages are exempt.

### 4.3 Walked line from pinned relations

`build-main-line` refuses a route whose `metadata.json` has no `osm.relations` (an Overpass name query pulls in every spur and variant; the 4,020 km Shikoku line came from 89 relations). The relation list is the section's trail and nothing else. The existing `refuseIncompleteLine` stays: a gap between two anchors in the way graph is a data error to fix (a wrong anchor or a missing relation), never spliced.
...
The recipe is the one the Camino Francés went through in PR #6:
...
Four dōjō sections replacing the ten non-chaining stages. About 45 day stages at 25–30 km, each ending at a temple (or at the last temple's town where the temple has no lodging), following the standard walking-guide pacing; the four sections chain (Awa ends at Temple 23, Tosa begins there). Distances are measured from the walked line, which is built from the henro trail relations only. The 88 temples are the places (they already carry coordinates and `stageIndex`, which is re-assigned to the new days); bangai temples and notable henro huts may be added as `cultural_site`. Text drafted (§6).

## 6. Drafted text and the review checklist

Where a stage has no `interior`, the agent drafts `theme` (2–4 words), `narrative` (2–3 sentences), and `reflection` (one closing line) grounded only in what the stage contains: its temples, shrines, passes, towns, distance, and climb. No legends, dates, or claims not present in the section's own `metadata.json`, `waypoints.geojson`, or OSM tags. Every drafted field is marked `"drafted": true` on the stage so it is greppable, and the mark is removed when the text is reviewed.

Each content PR carries `docs/review/<pilgrimage>.md` listing, per stage: endpoints with their OSM nodes, measured versus previously declared distance, the drafted text verbatim, and each curated place with its OSM id. The user edits stage files (or the checklist) before release; a release runbook step (`release.md`) refuses to tag while any `"drafted": true` remains.

## 7. Testing

- Unit: boundary advance/fail cases; section chaining; `build-main-line` refusing a relation-less route; `build-index` pilgrimages derivation and ordering; schema acceptance of the new fields.
- Data: every content PR ends with `build-ways && build-index && validate` deterministic and `check-site` in sync; the gate passes for every section it adds; no `"drafted": true` at release.
...
PR A (pipeline, no data change beyond regenerated reports) → PR B Norte → PR C Kumano → PR D Shikoku. Each PR is its own release immediately after merge, with the tag following the merge per `release.md` Phase 2b. PR B–D each depend on PR A; they do not depend on each other.
