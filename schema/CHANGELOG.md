...
- `index.schema.json`: an optional `stats` on a `pilgrimages[]` entry, copied verbatim from the sections' agreed block and absent for a pilgrimage whose sections carry none. Restated here only as far as the four keys a consumer may rely on, with `additionalProperties` open, so this file and `pilgrimage.schema.json` have no depths to drift over.
