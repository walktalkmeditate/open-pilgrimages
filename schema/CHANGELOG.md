...
### Breaking
- **`stages.schema.json`: `interior.reflection` is now required** when a stage carries an `interior` block. All 118 stages in the dataset already have one — the 113 under `routes/*/stages.json` plus the Camino Português coastal variant's 5, counted here because `validate` runs this schema over a variant's `stages.json` exactly as it does a route's; a contribution that omits it now fails validation instead of silently losing the line a walker reads at the end of the stage.
...
- `pilgrimage.schema.json`: an optional `pilgrimage.stats` object carrying counts measured over the whole walk — `lastUpdated`, `dataYear`, `dataNote` and `annualPilgrims` required when the block is present, plus optional `demographics`, `seasonalDistribution`, `modeOfTravel`, `infrastructure` and `sources`. Every section of the pilgrimage repeats it and `validate` refuses sections that disagree, exactly as `name`, `kind` and `circular` are repeated: spec §2 makes a pilgrimage index-level only, so it has no directory to hold a file of its own. The block is not a file either, so it carries neither `schemaVersion` nor `routeId`; `routes/{id}/stats.json` is unchanged and still holds the figures that really are one route's. `dataNote` is required rather than optional because a whole-circuit figure sitting in a section's metadata is one a reader takes for that section's own. The Shikoku 88's twenty-one years of Omotenashi Network walking completions are the first block of this kind — the file that held them went with `routes/shikoku-88/` when the pilgrimage became four dōjō.
- `index.schema.json`: an optional `stats` on a `pilgrimages[]` entry — `lastUpdated`, `dataYear`, `dataNote` and `annualPilgrims.latest`, absent for a pilgrimage whose sections carry none. A summary of the sections' agreed block rather than the block itself: `additionalProperties: false` at both depths is what keeps the year series, demographics and infrastructure out. `pilgrimages[]` exists so a consumer can see what a pilgrimage is without fetching its sections, and `index.json` is the one file every consumer downloads from `@main` before anything else — no route's own `stats.json` is copied into it either.
- `way-route.schema.json`: an optional `stampHours` object (`opens`, `closes`, both `HH:MM`) on a route's card, carried from the pilgrimage's `infrastructure.templeHours` where it declares one. It is the **stamp office's** hours, not the temple grounds': on the Shikoku henro the nōkyōjo shuts at 17:00, and a walker who arrives after it gets no stamp and must come back for one. Absent rather than empty where a pilgrimage declares nothing, which today is every route but the four dōjō. No ordering rule is imposed on the pair — an office whose hours span midnight is not this schema's business to refuse.
- `pilgrimage.schema.json`: an optional top-level `metadataOnly` string, present only on a section that deliberately ships with no walked line (spec §4.3) and saying in one sentence why. `check-site` used to infer that intent from a missing `route.geojson`, which reads the same on disk whether a section can never have geometry or simply never had `fetch-osm` run over it; the declaration is what separates the two. Single-purpose by design: a section with a line never carries it, which is why `osm.note` could not do this job — the Kohechi and the Nakahechi both carry one alongside a full line.
